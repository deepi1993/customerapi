...
app.post('/time_slots' , (req,res) => {
    ...
    TimeSlots.findOne({"serviceName": servicename} , function(err,timeslots) {
        if(err){
            var rs = {};
            rs.Success = false;
            rs.Message= err;
        ...
        if(timeslots === null){
            var rs = {};
            rs.Success = false;
            rs.Message = "no time slots found";
            return res.status(200).send(rs);
        }
        else{
            var rs ={};
            rs.Success = true;
            rs.Message = "Time slots for the required service";
            rs.TimeSlots = {};
            rs.TimeSlots.startTime = timeslots.startTime;
            rs.TimeSlots.endTime = timeslots.endTime;
            rs.TimeSlots.leadTime = timeslots.leadTime;
            rs.TimeSlots.interval = timeslots.interval;
